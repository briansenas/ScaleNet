...
def save_checkpoint(
    ...
    if epoch is not None:
        filename = os.path.join(
            checkpoint_path,
            filename.replace(".pth.tar", "_epoch%d_tid%d_.pth.tar" % (epoch, tid)),
        )
...
def eval_epoch_cvpr_RCNN(
    ...
    (
        eval_loss_list,
        eval_loss_horizon_list,
        eval_loss_pitch_list,
        eval_loss_roll_list,
        eval_loss_vfov_list,
    ) = ([], [], [], [], [])
    if opt.distributed:
        ...
        with tqdm(total=len(validation_loader)) as t:
            ...
            for i, (
                ...
                if if_loss:
                    ...
                    eval_loss_horizon_list.append(
                        loss_dict_reduced["loss_horizon"].item(),
                    )
                ...
                if i < 5 and if_vis:
                    _, return_dict = vis_utils.vis_SUN360(
                        tid,
                        savepath,
                        im_paths_SUN360,
                        output_horizon,
            ...
            if if_loss:
                ...
                if rank == 0:
                    ...
                    writer.add_histogram(
                        "loss/eval_loss_hist",
                        np.asarray(eval_loss_list),
                        tid,
                        bins="doane",
                    )
                    writer.add_histogram(
                        "loss/eval_loss_horizon_hist",
                        np.asarray(eval_loss_horizon_list),
                        tid,
                        bins="doane",
                    )
                    writer.add_histogram(
                        "loss/eval_loss_pitch_hist",
                        np.asarray(eval_loss_pitch_list),
                        tid,
                        bins="doane",
                    )
                    writer.add_histogram(
                        "loss/eval_loss_roll_hist",
                        np.asarray(eval_loss_roll_list),
                        tid,
                        bins="doane",
                    )
                    writer.add_histogram(
                        "loss/eval_loss_vfov_hist",
                        np.asarray(eval_loss_vfov_list),
                        tid,
                        bins="doane",
                    )
                    ...
                    if if_vis and rank == 0:
                        ...
                        pitch_all = merge_list_of_lists(
                            [
                                return_dict["pitch_list"]
                                for return_dict in return_dict_list
                            ],
                        )
                        roll_all = merge_list_of_lists(
                            [
                                return_dict["roll_list"]
                                for return_dict in return_dict_list
                            ],
                        )
                        vfov_all = merge_list_of_lists(
                            [
                                return_dict["vfov_list"]
                                for return_dict in return_dict_list
                            ],
                        )
                        f_mm_all = merge_list_of_lists(
                            [
                                return_dict["f_mm_list"]
                                for return_dict in return_dict_list
                            ],
                        )
                        ...
                        writer.add_histogram(
                            "dist/horizon_all",
                            np.asarray(horizon_all),
                            tid,
                            bins="doane",
                        )
                        writer.add_histogram(
                            "dist/pitch_all",
                            np.asarray(pitch_all) / np.pi * 180.0,
                            tid,
                            bins="doane",
                        )
                        writer.add_histogram(
                            "dist/roll_all",
                            np.asarray(roll_all) / np.pi * 180.0,
                            tid,
                            bins="doane",
                        )
                        writer.add_histogram(
                            "dist/vfov_all",
                            np.asarray(vfov_all) / np.pi * 180.0,
                            tid,
                            bins="doane",
                        )
                        writer.add_histogram(

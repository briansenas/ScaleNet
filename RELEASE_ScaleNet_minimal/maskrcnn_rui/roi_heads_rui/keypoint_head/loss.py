...
class KeypointRCNNLossComputation(object):
    ...
    def prepare_targets_for_gt_box_input(self, proposals, targets):
        labels = []
        keypoints = []
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            matched_targets = targets_per_image
    ...
    def prepare_targets(self, proposals, targets, if_debug=False):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            if if_debug:
                print(proposals_per_image.bbox.shape, targets_per_image.bbox.shape, matched_idxs, matched_idxs.shape)
    ...
    def subsample(self, proposals, targets, if_sample=True):
        """
    ...
    def __call__(self, proposals, keypoint_logits):
        ...
        if keypoint_targets.numel() == 0 or len(valid) == 0:
            return torch.zeros_like(keypoint_logits.sum(), device=keypoint_logits.sum().device)

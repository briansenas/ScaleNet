    ...
    def __init__(
        ...
        if self.if_roi_bbox_heads:
            ...
            self.roi_bbox_heads = build_roi_bbox_heads(
                ...
                opt,
                self.backbone.out_channels,
                if_roi_h_heads=self.if_roi_h_heads and self.if_shared_kps_head,
            )
    ...
    def forward(
        ...
        if self.if_roi_bbox_heads and input_data in ["coco", "IMDB-23K"]:
            if self.opt.est_bbox:
                ...
                proposals = [
                    box_list_kps_gt.copy_with_fields(["scores", "labels"])
                    for box_list_kps_gt in list_of_box_list_kps_gt
                ]
                targets_dup = [
                    box_list_kps_gt.copy_with_fields(["scores", "labels", "keypoints"])
                    for box_list_kps_gt in list_of_box_list_kps_gt
                ]
        ...
        if list_of_bbox_list_cpu is not None:
            list_of_bbox_list = [
                bbox_list_array.to(self.device)
                for bbox_list_array in list_of_bbox_list_cpu
            ]
            list_of_bbox_list = [
                bbox_list.resize(size)
                for bbox_list, size in zip(
                    list_of_bbox_list,
                    image_sizes_after_transform,
                )
            ]
                ...
                return_dict.update(
                    {
                        ...
                        "class_person_H_logits_softmax": class_logits_softmax,
                        "class_person_H_logits": class_logits,
        ...
        if list_of_oneLargeBbox_list is not None and self.if_classifier_heads:
            ...
            return_dict.update(
                {
                    "output_horizon": cls_outputs["output_horizon"]["class_logits"],
                    "output_pitch": cls_outputs["output_pitch"]["class_logits"],
                    "output_roll": cls_outputs["output_roll"]["class_logits"],
        ...
        for kps in kps_batch:
            ...
            for which_side in ["left", "right"]:
                which_side_vis_list.append(
                    kps[2, dataset_keypoints.index(which_side + "_hip")] > kp_thresh
                    and kps[2, dataset_keypoints.index(which_side + "_ankle")]
                    > kp_thresh
                    and kps[2, dataset_keypoints.index(which_side + "_knee")]
                    > kp_thresh,
                )
    ...
    for l in range(len(kp_lines)):
        ...
        if kps[2, i1] > kp_thresh:
            ...
            if (not if_fake_scores) and if_show_kps_score:
                cv2.putText(
                    kp_mask,
                    dataset_keypoints[i1] + " %.2f" % kps[2, i1],
                    p1,
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.3,
                    (255, 255, 255),
        ...
        if kps[2, i2] > kp_thresh:
            ...
            if (not if_fake_scores) and if_show_kps_score:
                cv2.putText(
                    kp_mask,
                    dataset_keypoints[i2] + " %.2f" % kps[2, i2],
                    p2,
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.3,
                    (255, 255, 255),

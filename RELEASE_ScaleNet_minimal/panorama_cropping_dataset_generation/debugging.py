...
def plotPerformance(output_dir, data_pitch):
    ...
    plt.ylabel("% Correct")
    plt.savefig(
        os.path.join(output_dir, "pitch_error.png"),
        bbox_inches="tight",
        dpi=150,
    )
    plt.close()
...
def showHorizonLine(
    ...
    if debug:
        if GT == False:
            draw.text(
                (0, 0),
                f"vfov:{vfov*180/np.pi:.2f}, pitch:{pitch*180/np.pi:.2f}, roll:{roll*180/np.pi:.2f}, f_mm:{focal_length:.2f}",
                (255, 255, 255),
            )
        else:
            draw.text(
                (0, h - 12),
                f"GT: vfov:{vfov*180/np.pi:.2f}, pitch:{pitch*180/np.pi:.2f}, roll:{roll*180/np.pi:.2f}, f_mm:{focal_length:.2f}",
                (255, 255, 255),
            )

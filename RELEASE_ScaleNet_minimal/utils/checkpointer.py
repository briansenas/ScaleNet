...
class Checkpointer:
    ...
    def save(self, name, **kwargs):
        ...
        self.logger.info(
            colored(f"Saving checkpoint to {save_file}", "white", "on_magenta"),
        )
    ...
    def load(
        ...
        current_keys, loaded_keys = self._load_model(
            checkpoint,
            self.logger,
            skip_kws=skip_kws,
    ...
    def get_checkpoint_file(self, task_name=None):
        if task_name is not None:
            ...
            if len(task_name_splits) == 1:
                ...
                ckpt_folder = os.path.join(self.opt.checkpoints_folder, task_name)
        else:
    ...
    def _load_model(
        ...
        current_keys, loaded_keys = load_state_dict(
            self.model,

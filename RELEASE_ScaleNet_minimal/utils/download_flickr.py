...
print(f"Loading variables from: {env_path}")
...
class Flickr:
    ...
    def get_images_from_ids(self, image_ids):
        ...
        for photo_id in tqdm(image_ids):
            try:
                ...
                # If a photo_id is now not available just ignore
        ...
        print(len(failed), "failed to be fetched...")
        return images, failed

    def get_images_urls_metadata(
        self, group_id=None, tag=None, num_images=50000, extras="url_o", page_id=None
    ):
        """
        ...
        for page in tqdm(page_list):
            if group_id is not None:
                photos = self.api.groups.pools.getPhotos(
                    group_id=group_id, extras=extras, page=page
                )
            elif tag is not None:
                photos = self.api.photos.search(
                    tags=tag, extras=extras, page=page, per_page=per_page
                )
            else:
                raise ValueError
            ...
            print(
                f"[PAGE {page}] Number of unique images so far: {len(list(set([x[0] for x in images])))}"
            )
    ...
    def download(
        ...
        failed = []
        if group_id is not None:
            images = self.get_images_urls_metadata(
                group_id=group_id, num_images=num_images, extras=extras, page_id=page_id
            )
        elif tag is not None:
            images = self.get_images_urls_metadata(
                tag=tag, num_images=num_images, extras=extras, page_id=page_id
            )
        elif image_ids is not None:
            images, failed = self.get_images_from_ids(image_ids)
        else:
            raise ValueError

        if not os.path.exists(PICKLE_DIR):
            os.makedirs(PICKLE_DIR, exist_ok=True)
        images_dir = os.path.join(PICKLE_DIR, f"flickr_image_{iter}_{num_images}.pkl")
        failed_dir = os.path.join(PICKLE_DIR, f"flickr_failed_{iter}_{num_images}.pkl")
        joblib.dump(images, images_dir)
        if failed:
            joblib.dump(failed, failed_dir)
...
def scrape_and_download():
    dataset_folder = os.path.join(DATADIR, "flickr_pano_images")
    ...
    download_type = "tag"
    flickr = Flickr(
        API_KEY, API_SECRET, download_with_exif_only=True, download_original=True
    )
    ...
    if download_type == "group":
        ...
        destination_dir = os.path.join(dataset_folder, f"group_{group_name}_cluster")
        ...
        destination_dir = os.path.join(dataset_folder, f"tag_{tag}_cluster")
        os.makedirs(destination_dir, exist_ok=True)
...
def iterate_and_download(flickr_image_ids, overwrite=False):
    dataset_folder = os.path.join(DATADIR, "flickr_pano_images")
    ...
    extras = "url_o"
    flickr = Flickr(
        API_KEY, API_SECRET, download_with_exif_only=False, download_original=False
    )
    # Do batches of 100 or something and append ids that have been download to be able to return gracefully if 429
    iter = 0
    for ids in tqdm(
        np.array_split(flickr_image_ids, len(flickr_image_ids) // num_images)
    ):
        if os.path.exists(
            os.path.join(PICKLE_DIR, f"flickr_image_{iter}_{num_images}.pkl")
        ):
            print(f"The file for iteration {iter} was found, skipping...")
            iter += 1
            continue
        flickr.download(
            num_images=num_images,
            extras=extras,
            destination_dir=dataset_folder,
            image_ids=ids,
            iter=iter,
        )
        iter += 1
        # Sleep for 1 minute to avoid >3600 requests per hour
        time.sleep(10)


def download():
    flickr_image_ids = np.load(os.path.join(DATADIR, "flickr_photo_ids.npy"))
    iterate_and_download(flickr_image_ids)


def download_missing_ids():
    with open(os.path.join(DATADIR, "empty_ids_flickr.txt"), "r") as file:
        flickr_image_ids = file.read().strip("\n").split("\n")
    flickr_image_ids = list(map(lambda x: x.strip(".jpg"), flickr_image_ids))
    iterate_and_download(flickr_image_ids)


DATADIR = "data/pano360"
PICKLE_DIR = os.path.join(DATADIR, "pickle")
# PICKLE_DIR = os.path.join(DATADIR, "pickle_empty")
if __name__ == "__main__":
    download()
    # download_missing_ids()
    # scrape_and_download()

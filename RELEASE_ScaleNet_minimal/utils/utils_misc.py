    ...
    for idx in range(batch_dict["num_samples"]):
        ...
        for key in batch_dict:
            ...
            if key in [
                "yc_est_list",
                "person_hs_est_list",
                "vt_camEst_N_delta_est_list",
                "f_pixels_est_mm_list",
                "v0_est_list",
            ]:
                new_dict[key] = [
                    content_layer[idx] for content_layer in content
                ]  # index by layer to index by sample
            else:
...
def softmax_with_bins(input, bins):
    est_batch = (nn.functional.softmax(input, dim=1) * bins).sum(
        dim=1,
    )  # sum is reducing dims; [N]
    return est_batch
...
def pad_zeros_to_good_num(bboxes, good_num):
    ...
    if N_to_pad > 0:
        return torch.cat(
            (bboxes, torch.zeros((N_to_pad, bboxes.shape[1]), device=bboxes.device)),
        )
    else:
        return bboxes


def list_of_bboxes_to_bboxes_padded(
    list_of_bbox,
    good_num,
    H_batch_array,
...
def pad_bbox(bboxes, max_length):
    ...
    assert bboxes.shape[0] <= max_length, "bboxes length of %d > max_length of %d!" % (
        bboxes.shape[0],
        max_length,
    )
